...
if ((REBUILD)) || ! docker image inspect b02-gw4t4 >/dev/null 2>&1; then
  docker build -t b02-gw4t4 -f "$HERE/Dockerfile" "$HERE" >/dev/null
fi
...
docker run -d --name b02-gateway4t4 --network b02-4t4-net --cap-add NET_ADMIN \
  ...
  b02-gw4t4 --listen 9710 --downstream "${BRIDGE_IP}:9711" >/dev/null

docker run -d --name b02-bgserver4t4 --network b02-4t4-net \
  --entrypoint iperf3 b02-gw4t4 -s -p 5211 >/dev/null
