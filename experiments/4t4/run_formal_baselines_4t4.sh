...
KV=104544
COMMON=(--out-dir "$OUT" --frozen-manifest "$MANIFEST" --instances 4 --repetitions 5 --n-requests 120 --warmup 24 --concurrency 4 --output-tokens 4 --kv-cache-tokens "$KV" --rhos 0.5,0.8,1.0,1.2 --policies FullSync,RateFIFO,LatestOnly,AgeCov-Greedy,StaticSemantic,Adaptive --global-topk 16 --relay-max-inflight 4 --rate-burst-frames 4 --cooldown-s 1.0)
