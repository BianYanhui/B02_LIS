#!/usr/bin/env python3
"""Sensitivity supplement on the isolated 4xT4 live path.
...
async def run_sensitivity_matrix(args, root, do_cell, order_by_rep) -> None:
    rho = float(args.rhos.split(",")[0])
    for rep in range(args.repetitions):
        ...
        plan: list[tuple] = [(IDEAL_POLICY, None, False, "", None, None)]
        if args.stage == "panel_a":
            plan.append(("StaticSemantic", rho, False, "", None, None))
            for c0 in PANEL_A_C0:
                for theta in PANEL_A_THETA:
                    plan.append(("Adaptive", rho, False, f"_c0{c0}_theta{theta}", c0, theta))
        elif args.stage == "panel_b":
            for policy in ("RateFIFO", "StaticSemantic", "Adaptive"):
                suffix = "_c01024_theta30" if policy == "Adaptive" else ""
                plan.append((policy, rho, False, suffix, 1024 if policy == "Adaptive" else None, 30.0 if policy == "Adaptive" else None))
        elif args.stage == "trigger":
            plan.append(("StaticSemantic", rho, False, "", None, None))
            for gate in (1.0, 2.0, 4.0):
                plan.append(("Adaptive", rho, False, f"_c01024_theta30_gate{gate:g}", 1024, 30.0))
        else:
        ...
        if rep == 0:
            ...
            order = list(plan)
            random.Random(stable_int(args.seed, "cell-order", args.stage, rep)).shuffle(order)
        order_by_rep[rep] = [
            IDEAL_POLICY if policy == IDEAL_POLICY else f"{policy}@rho{rho_value}" + suffix
            for policy, rho_value, _bg, suffix, _c0, _theta in order
        ]
        for order_index, (policy, rho_value, bg, suffix, c0, theta) in enumerate(order):
            apply_admission(args, c0, theta)
            if args.stage == "trigger" and policy == "Adaptive":
                args.gate = float(suffix.rsplit("_gate", 1)[-1])
            else:
                args.gate = 2.0
...
async def run(args: argparse.Namespace) -> dict:
    ...
    if args.smoke:
        ...
        trace = make_trace(trace_path, rep, args)
        trace_hash = sha256_file(trace_path)
        order_by_rep[rep] = [IDEAL_POLICY, "StaticSemantic@rho1.2", "Adaptive@rho1.2_c01024_theta30"]
        ...
        for idx, (policy, rho, bg, suffix) in enumerate([
            ("StaticSemantic", 1.2, False, ""), ("Adaptive", 1.2, False, "_c01024_theta30"),
        ], start=1):
            await do_cell(rep, policy, rho, bg, trace, trace_hash, idx, ",".join(order_by_rep[rep]), suffix)
    else:
        await run_sensitivity_matrix(args, root, do_cell, order_by_rep)
...
def smoke_report(cells: list[dict], checks: list[dict]) -> list[dict]:
    ...
    full = by_id.get("StaticSemantic@rho1.2", {})
    adaptive = by_id.get("Adaptive@rho1.2_c01024_theta30", {})
...
def main() -> None:
    ...
    parser.add_argument("--out-dir", default="/home/byh/B02/supplemental_20260918_sensitivity")
    ...
    parser.add_argument("--stage", choices=["smoke", "panel_a", "panel_b", "trigger", "custom"], default="panel_a")
    ...
    parser.add_argument("--rate-burst-frames", type=int, default=1,
