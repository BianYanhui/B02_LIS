#!/usr/bin/env bash
# Build the isolated b02-4t4-net docker network, image, gateway + bgserver
...
if ((REBUILD)) || ! docker image inspect b02-gw4t4-sens >/dev/null 2>&1; then
  docker build -t b02-gw4t4-sens -f "$HERE/Dockerfile" "$HERE" >/dev/null
fi
...
docker run -d --name b02-gateway4t4 --network b02-4t4-net --cap-add NET_ADMIN \
  ...
  b02-gw4t4-sens --listen 9710 --downstream "${BRIDGE_IP}:9711" >/dev/null

docker run -d --name b02-bgserver4t4 --network b02-4t4-net \
  --entrypoint iperf3 b02-gw4t4-sens -s -p 5211 >/dev/null
...
docker exec b02-gateway4t4 sh -c "
  ...
  # 9710.  Keep reset/stats acknowledgements out of the saturating iperf3
...
echo "b02-4t4-net ready: bridge=${BRIDGE_IP} gateway+bgserver running, sig=${SIG_BIT}bit/s netem=${NETEM_DELAY}ms mtu=${MTU}"
