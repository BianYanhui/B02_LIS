#!/usr/bin/env python3
"""Control-plane delay-anchor supplement on the isolated 4xT4 live path.

Fork of the frozen 4xT4 harness. Does not enforce experiment_manifest.json.
Per-cell CONFIG carries tau/lambda/gate/queue_gate so Adaptive grids do not
restart the gateway.
...
def reuse_prefix_lengths(pool_size: int, seed: int, prefix_length: int = 0) -> list[int]:
    """Paper mix (1K/2K/4K = 35/40/25) or a single prefix length for panel (b)."""
    if prefix_length > 0:
        return [prefix_length] * pool_size
...
def make_trace(path: Path, rep: int, args: argparse.Namespace) -> list[TraceRequest]:
    ...
    prefix_lengths = {
        phase: reuse_prefix_lengths(
            args.pool_size,
            stable_int(args.seed, args.workload, rep, phase, "prefix-lengths", getattr(args, "prefix_length", 0)),
            prefix_length=int(getattr(args, "prefix_length", 0) or 0),
        )
        for phase in (0, 1)
    }
...
class Dispatcher:
    def __init__(self, j: int, prefill_tokens_per_ms: float, queue_penalty_ms: float, guard_ms: float) -> None:
        ...
        self.rr = 0
        self.apply_upsert_s: list[float] = []
    ...
    def apply_upsert(self, instance: int, digest: str, coverage: int, generated_at: float, received_at: float) -> None:
        ...
        self.apply_upsert_s.append(time.perf_counter() - started)
...
class LinkRuntime:
    ...
    async def configure_cell(self, dispatcher: Dispatcher, cell_id: int, digest_map: dict[int, str], policy: str,
                             ...
                             rate_burst_frames: int, tau: float = 30.0, util_lambda: float = 16.0,
                             delay_gate: float = 2.0, queue_gate: int = 8) -> None:
        ...
        self.received = 0
        self.update_events = []
        drain_fps = float(getattr(self, "_drain_fps", 0.0) or 0.0)
        if drain_fps > 0:
            self.rate_milliframes_per_s = max(1, int(round(drain_fps * 1000.0)))
        else:
            self.rate_milliframes_per_s = 0
        self.rate_burst_frames = rate_burst_frames
        self.tau = tau
        self.util_lambda = util_lambda
        self.delay_gate = delay_gate
        ...
        tau_ms = max(0, int(round(float(self.tau) * 1000.0)))
        lambda_x100 = max(0, int(round(float(self.util_lambda) * 100.0)))
        gate_ds = max(0, int(round(float(self.delay_gate) * 10.0)))
        queue_gate = max(0, int(self.queue_gate))
        extra = CFG_EXTRA.pack(min(tau_ms, 65535), min(lambda_x100, 65535), min(gate_ds, 255), min(queue_gate, 255))
        self.agent_writers[0].write(HDR.pack(K_CONFIG, 0, cell_id, 0, 0, 0, time.time()) + (cfg_payload + extra).ljust(32, b"\x00"))
...
async def run_cell(trace: list[TraceRequest], policy: str, rho: float | None, bg: bool, cell_tag: str,
    ...
    cluster_n = int(getattr(args, "cluster_n", 4) or 4)
    drain_fps = float(getattr(args, "drain_fps", 0.0) or 0.0)
    sync_s = float(getattr(args, "sync_s", 1.0) or 1.0)
    fanin_frames_sent = 0
    if not is_ideal:
        sig_bit = 100_000_000
        ...
        link._drain_fps = drain_fps
        await link.configure_cell(
            dispatcher, cell_uid % 60000, digest_map, policy, args.global_topk,
            args.relay_max_inflight, sig_bit, args.rate_burst_frames,
            tau=float(getattr(args, "tau", 30.0)),
            util_lambda=float(getattr(args, "util_lambda", 16.0)),
            delay_gate=float(getattr(args, "gate", 2.0)),
            queue_gate=int(getattr(args, "adaptive_queue_gate", 8)),
        )
    ...
    async with aiohttp.ClientSession() as session:
        ...
        fanin_stop = asyncio.Event()

        async def fanin_loop() -> None:
            nonlocal fanin_frames_sent
            if is_ideal or cluster_n <= 4:
                return
            tick = 0
            scale = max(1, cluster_n // 4)
            interval = max(0.02, sync_s / scale)
            extra = max(0, cluster_n - 4)
            while not fanin_stop.is_set():
                tick += 1
                try:
                    for inst in range(len(URLS)):
                        for digest, cov in list(advertised[inst].items()):
                            if fanin_stop.is_set():
                                return
                            link.send(K_UP, inst, digest, cov)
                            fanin_frames_sent += 1
                    ...
                    return
                try:
                    await asyncio.wait_for(fanin_stop.wait(), timeout=interval)
                    return
                except asyncio.TimeoutError:
                    continue

        fanin_task = asyncio.create_task(fanin_loop())
    ...
    if not is_ideal:
        fanin_stop.set()
        try:
            await asyncio.wait_for(fanin_task, timeout=2.0)
        ...
        net_metrics = {
            ...
            "cluster_n": cluster_n, "drain_fps": drain_fps, "sync_s": sync_s,
            "fanin_frames_sent": fanin_frames_sent,
            ...
            "c0_tokens": (float(args.util_lambda) * FRAME) if policy == "Adaptive" else "",
            "theta_s": float(args.tau) if policy == "Adaptive" else "",
            "util_lambda": float(args.util_lambda) if policy == "Adaptive" else "",
            "delay_gate_s": float(args.gate),
            "queue_gate_frames": int(args.adaptive_queue_gate),
            "prefix_length": int(getattr(args, "prefix_length", 0) or 0),
    ...
    n = len(records)
    apply_s = list(getattr(dispatcher, "apply_upsert_s", []))
    metrics = {
        "request_count": n,
        ...
        "sync_s": sync_s,
        "fanin_frames_sent": fanin_frames_sent,
        "apply_upsert_count": len(apply_s),
        "apply_upsert_p50_us": (percentile(apply_s, 50) * 1e6) if apply_s else 0.0,
        "apply_upsert_p95_us": (percentile(apply_s, 95) * 1e6) if apply_s else 0.0,
...
PANEL_A_C0 = (256, 512, 1024, 2048)
PANEL_A_THETA = (10, 20, 30, 60)


def apply_admission(args: argparse.Namespace, c0: int | None, theta: float | None) -> None:
    if c0 is None:
        args.util_lambda = 16.0
        args.tau = 30.0
        return
    args.util_lambda = float(c0) / float(FRAME)
    args.tau = float(theta if theta is not None else 30.0)


async def run_controlplane_matrix(args, root, do_cell, order_by_rep) -> None:
    rho = 0.0
    cluster_ns = [int(value) for value in str(args.cluster_ns).split(",") if value.strip()]
    ...
    for rep in range(args.repetitions):
        trace_path = root / "raw" / "traces" / f"{args.workload}_L{int(args.prefix_length or 0)}_trace_rep{rep}.csv"
        trace = make_trace(trace_path, rep, args)
        trace_hash = sha256_file(trace_path)
        plan: list[tuple] = [(IDEAL_POLICY, None, False, "", 4)]
        if args.stage in {"calibrate", "smoke_n4"}:
            args.drain_fps = 0.0
            plan += [("StaticSemantic", rho, False, "", 4), ("Adaptive", rho, False, "", 4)]
        elif args.stage == "smoke_n32":
            args.drain_fps = float(args.drain_fps or 200.0)
            plan += [(policy, rho, False, "", 32) for policy in ["RateFIFO", "StaticSemantic", "Adaptive"]]
        else:
            for cluster_n in cluster_ns:
                for policy in policies:
                    plan.append((policy, rho, False, "", cluster_n))
        order = list(plan)
        if args.stage == "fanin":
            head, tail = order[:1], order[1:]
            random.Random(stable_int(args.seed, "cell-order", args.stage, rep)).shuffle(tail)
            order = head + tail
        order_by_rep[rep] = [
            IDEAL_POLICY if policy == IDEAL_POLICY else f"{policy}@N{cluster_n}_mu{int(args.drain_fps or 0)}"
            for policy, _rho, _bg, _suffix, cluster_n in order
        ]
        for order_index, (policy, rho_value, bg, suffix, cluster_n) in enumerate(order):
            args.cluster_n = cluster_n
            if policy == "Adaptive":
                apply_admission(args, 1024, 30.0)
            else:
                apply_admission(args, None, None)
            await do_cell(rep, policy, rho_value, bg, trace, trace_hash, order_index, ",".join(order_by_rep[rep]), suffix)
...
async def run(args: argparse.Namespace) -> dict:
    ...
    async def do_cell(rep: int, policy: str, rho: float | None, bg: bool, trace: list[TraceRequest], trace_hash: str, order_index: int, order_seq: str, suffix: str = "") -> dict:
        ...
        drain_fps = float(getattr(args, "drain_fps", 0.0) or 0.0)
        if policy == IDEAL_POLICY:
            cell_id = IDEAL_POLICY
        else:
            cell_id = f"{policy}@N{cluster_n}_mu{int(drain_fps)}" + ("+bg" if bg else "") + suffix
    ...
    await run_controlplane_matrix(args, root, do_cell, order_by_rep)
...
def smoke_report(cells: list[dict], checks: list[dict]) -> list[dict]:
    ...
    by_policy = {row["policy"]: row for row in cells}
    full = by_policy.get("StaticSemantic", {})
    adaptive = by_policy.get("Adaptive", {})
    ideal = by_policy.get(IDEAL_POLICY, {})
...
def main() -> None:
    ...
    parser.add_argument("--out-dir", default="/home/byh/B02/supplemental_20260920_controlplane")
    ...
    parser.add_argument("--stage", choices=["calibrate", "smoke_n4", "smoke_n32", "fanin", "custom"], default="calibrate")
    ...
    parser.add_argument("--rhos", default="1.2")
    parser.add_argument("--policies", default="RateFIFO,StaticSemantic,Adaptive",
    ...
    parser.add_argument("--relay-max-inflight", type=int, default=32,
                        help="shared ACK-delimited gateway frame window for every real policy")
    parser.add_argument("--rate-burst-frames", type=int, default=20,
    ...
    parser.add_argument("--cluster-n", type=int, default=4, help="logical worker fan-in; 4 is the physical site")
    parser.add_argument("--cluster-ns", default="4,16,32,64", help="fanin-stage N grid")
    parser.add_argument("--drain-fps", type=float, default=200.0, help="control-plane ingest μ in frames/s; 0=unlimited")
    parser.add_argument("--sync-s", type=float, default=1.0, help="cache-index heartbeat period at N=4")
    parser.add_argument("--netem-ms", type=int, default=40, help="recorded RTT; setup_net.sh applies it")
    parser.add_argument("--tau", type=float, default=30.0, help="freshness e-folding horizon seconds")
    parser.add_argument("--util-lambda", type=float, default=16.0, help="utility byte cost; C0=lambda*64")
    parser.add_argument("--gate", type=float, default=2.0, help="EWMA delay threshold that arms Adaptive")
    parser.add_argument("--adaptive-queue-gate", type=int, default=8, help="queued frames that arm Adaptive")
    parser.add_argument("--prefix-length", type=int, default=0,
                        help="if >0, every lineage uses this prefix length; 0 keeps the 1K/2K/4K mix")
    ...
    if args.smoke:
        args.repetitions = 1
        args.tag = args.tag if args.tag != "formal" else "smoke_n4"
        args.stage = "smoke_n4"
        args.tau = 30.0
        args.util_lambda = 16.0
        args.drain_fps = 0.0
    ...
    if args.stage in {"calibrate", "smoke_n4"}:
        ...
        args.drain_fps = 0.0
        args.cluster_n = 4
    if args.stage == "smoke_n32":
        args.repetitions = 1
        args.drain_fps = args.drain_fps if args.drain_fps else 200.0
        args.cluster_n = 32
    ...
    # Sensitivity runs must not bind to the frozen 4T4 manifest.
    started = time.time()
    ...
    generated_lt_forwarded = sum(
        int(row.get("upserts_generated", 0)) + int(row.get("source_tombstones_sent", 0)) + int(row.get("fanin_frames_sent", 0))
        < int(row.get("net_msgs_delivered", 0))
        for row in nonideal
    )
    ...
    if args.smoke or args.stage in {"calibrate", "smoke_n4", "smoke_n32"}:
